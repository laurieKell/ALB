...
double sr::recruits(int istock, double ssb, int iyr, int iunit, int iseason, int iarea, int iter)
   ...
   switch((int)model[istock][_yr]) 
      ...
      // Re-used as SS3 BevHolt
         returnval = (4 * param(istock,1,_yr,iunit,iseason,iarea,iter) * param(istock,2,_yr,iunit,iseason,iarea,iter) * ssb * 0.5) / (param(istock,3,_yr,iunit,iseason,iarea,iter) * (1-param(istock,1,_yr,iunit,iseason,iarea,iter)) + ssb * 0.5 * (5 * param(istock,1,_yr,iunit,iseason,iarea,iter) - 1));
